...
export default async function handler(req,res) {
 ...
 if (req.method === 'POST' && !sameOrigin(req)) return res.status(403).json({message:'Request origin not allowed'});
 ...
 const body = req.body || {};
 if (JSON.stringify(body).length > 700000) return res.status(413).json({message:'Content is too large'});
 try {
  if(action === 'login' && req.method === 'POST') {
   if(typeof body.email !== 'string' || typeof body.password !== 'string' || body.email.length>254 || body.password.length>256) return res.status(400).json({message:'Enter your email and password'});
   const ip=String(req.headers['x-forwarded-for']||req.socket?.remoteAddress||'unknown').split(',')[0].trim();
   const bucket=crypto.createHmac('sha256',process.env.SUPABASE_SECRET_KEY).update('login:'+ip+':'+body.email.toLowerCase()).digest('hex');
   const allowed=await sb('/rest/v1/rpc/nl_accept_attempt',{method:'POST',body:JSON.stringify({bucket})});
   if(!allowed)return res.status(429).json({message:'Too many login attempts. Try again in ten minutes.'});
   let login;
   try {login = await sb('/auth/v1/token?grant_type=password',{auth:true,method:'POST',body:JSON.stringify({email:body.email,password:body.password})});} catch {return res.status(401).json({message:'Login failed. Check your credentials or try again later.'});}
  ...
  if(action === 'logout' && req.method === 'POST') {
   const token=cookie(req); setSession(res,'',0);
  ...
  if(action === 'content') {
   ...
   const saved = await sb(`/rest/v1/nl_content?key=eq.${key}&version=eq.${version}`, {method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({items,version:version+1,updated_at:new Date().toISOString()})});
  ...
  if(action === 'leads') {
   ...
   if(!/^[a-f0-9-]{36}$/.test(body.id) || !['New','Contacted','In progress','Completed','Archived'].includes(body.status) || typeof body.notes!=='string' || body.notes.length>3000) return res.status(400).json({message:'Invalid lead update'});
   const saved=await sb(`/rest/v1/nl_leads?id=eq.${body.id}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({status:body.status,notes:body.notes})});
   if(!saved.length) return res.status(404).json({message:'Lead not found'});
   return res.json(saved[0]);
  }
  return res.status(404).json({message:'Action not found'});
 } catch {return res.status(502).json({message:'Unable to reach your database. Check setup or try again.'});}
}
